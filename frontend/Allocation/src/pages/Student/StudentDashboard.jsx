...
const DashboardCards = ({
  ...
  const renderCard = (project, title, type) => {
    ...
    if (type === "idea") {
      ...
      mentorDisplay =
        project.mentorName || project.approvedMentor?.name || "Pending";
    ...
    const statusColor = statusText.toLowerCase().includes("pending")
      ...
      : statusText.toLowerCase().includes("approve") ||
    ...
    if (type === "idea") {
      teamLeadName =
        project.teamLead?.name || project.teamMembers?.[0]?.name || "N/A";
      teamLeadEmail =
        project.teamLead?.email || project.teamMembers?.[0]?.email || "-";
    } else {
      teamLeadName =
        ...
        "N/A";
      teamLeadEmail =
        project.assignedTeamLead?.email ||
        project.teamMembers?.[0]?.email ||
        "-";
    }
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col h-[500px] p-5 w-full">
        <div className="flex-1 overflow-y-auto pr-1">
          ...
          {/* Interview Info */}
          {project.status === "interview_scheduled" && project.interview && (
            <div className="mb-3 space-y-1">
              ...
              <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                <p className="text-sm">
                  ...
                  {project.interview.date || "N/A"}
                </p>

                <p className="text-sm">
                  <span className="font-medium">Time:</span>{" "}
                  {project.interview.time || "N/A"}
                </p>

                <p className="text-sm">
                  <span className="font-medium">Location:</span>{" "}
                  {project.interview.location || "N/A"}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="mb-3 space-y-1">
            ...
            {mentorEmail && (
              <p className="text-xs text-gray-500">{mentorEmail}</p>
            )}
          </div>
          ...
          <div className="bg-blue-50 p-3 rounded-lg mb-3">
            ...
            {teamLeadEmail && (
              <p className="text-xs text-gray-500">{teamLeadEmail}</p>
            )}
          </div>
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      ...
      setError("");
      try {
        const [assigned, idea] = await Promise.all([
          getMyAssignedProject(),
          getMyIdeaProject(),
        ]);
        ...
        if (activeProject?._id) {
          const [checklistRes, form1Res, form2Res, form3Res] =
            await Promise.all([
          ...
          setChecklist(
            Array.isArray(checklistRes.data?.checklist)
