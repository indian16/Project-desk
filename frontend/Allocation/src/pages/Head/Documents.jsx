              ...
              {documents.map((doc) => (
                ...
                >
                  <div className="flex items-center gap-3 sm:gap-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 text-sm sm:text-base">{doc.title}</p>
                      <p className="text-xs sm:text-sm text-gray-400 uppercase">{doc.fileName.split(".").pop()} File</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 sm:gap-2 relative">
                    ...
                    <button
                      onClick={() => { setConfirmDeleteId(doc._id); setDeleteType("doc"); }}
                      className="p-1 sm:p-2 text-slate-400 hover:text-red-600"
                    >
                      <Trash2 size={18} />
                    </button>
                    {confirmDeleteId === doc._id && deleteType === "doc" && (
                      <div className="absolute right-0 top-10 z-20 bg-white shadow-md rounded-xl p-1 border-2 border-slate-100 flex gap-1">
                        <button onClick={() => handleDelete(doc._id)} className="bg-red-600 text-white p-2 rounded-lg"><Check size={16} /></button>
                        <button onClick={() => setConfirmDeleteId(null)} className="bg-slate-200 p-2 rounded-lg"><X size={16} /></button>
                      </div>
                    )}
                  </div>
                </motion.div>
      ...
      <div className="bg-slate-50 flex-1 rounded-b-[1rem] border-x-2 border-b-2 border-slate-200 shadow-lg flex flex-col overflow-hidden">
        <div className="p-4 sm:p-6 bg-white border-b-2 border-slate-100 m-4 rounded-2xl shadow-sm">
          <div className="flex gap-2 items-center">
            ...
            />
            <button onClick={handleAddChecklist} className="bg-slate-900 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl flex items-center justify-center">
              <Plus />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3">
          <AnimatePresence mode="popLayout" initial={false}>
            {checklists.map((item) => (
              ...
              >
                <span className="font-bold text-slate-800 text-sm sm:text-base">{item.title}</span>
                <div className="relative">
                  <button onClick={() => { setConfirmDeleteId(item._id); setDeleteType("check"); }} className="text-slate-400 hover:text-red-500 p-1 sm:p-2"><Trash2 size={18} /></button>
                  {confirmDeleteId === item._id && deleteType === "check" && (
                    <div className="absolute right-0 top-10 z-20 bg-white shadow-md rounded-xl p-1 border-2 border-slate-100 flex gap-1">
                      <button onClick={() => handleDelete(item._id)} className="bg-red-600 text-white p-2 rounded-lg"><Check size={16} /></button>
                      <button onClick={() => setConfirmDeleteId(null)} className="bg-slate-200 p-2 rounded-lg"><X size={16} /></button>
                    </div>
                  )}
                </div>
