...
const ReviewedIdeas = ({ academicYear }) => {
  ...
  useEffect(() => {
    const fetchIdeas = async () => {
      try {
        ...
        const approvedIdeas = data.filter((idea) => idea.status === "approved_by_head");
        const rejectedIdeas = data.filter((idea) => idea.status === "rejected_by_head");
      ...
      <div className="pl-3">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${
            type === 'accepted' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'
          }`}>
        ...
        {idea.headRemarks && (
          <div className="mt-4 pt-3 border-t border-slate-50">
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tight mb-1">Head Remarks</p>
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-20 text-slate-400">
      ...
      <p className="text-xs font-bold uppercase tracking-widest">Compiling Archive...</p>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10">
      ...
      <div className="flex items-center gap-4">
        <h2 className="text-xl font-black text-slate-800 tracking-tight">Review History</h2>
